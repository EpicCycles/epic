...
describe('basic render', () => {
    ...
    test('should show the bike name and bike edit page when rendered', () => {
        ...
        expect(findDataTest(component, 'reset-bike').length).toBe(0);
        expect(findDataTest(component, 'save-bike').length).toBe(0);
        expect(findDataTest(component, 'delete-bike').length).toBe(1);
        expect(findDataTest(component, 'add-bike-part').length).toBe(0);
    });
    ...
    test('should not show save if field changed but invalid', () => {
        ...
        expect(findDataTest(component, 'reset-bike').length).toBe(1);
        expect(findDataTest(component, 'save-bike').length).toBe(0);
        expect(findDataTest(component, 'delete-bike').length).toBe(1);
        expect(findDataTest(component, 'add-bike-part').length).toBe(0);
    });
    ...
    test('should call save function passed', () => {
        ...
        component.update();
        expect(findDataTest(component, 'reset-bike').length).toBe(1);
        expect(findDataTest(component, 'save-bike').length).toBe(1);
        expect(findDataTest(component, 'delete-bike').length).toBe(1);
        expect(findDataTest(component, 'add-bike-part').length).toBe(0);
        ...
        expect(saveBike).toHaveBeenCalledWith(expect.objectContaining({
            "id": 58,
            "model_name": "4",
            "description": null,
            "colours": "anthracite/black/lime",
...
describe('With add part', () => {
    ...
    test('should display add part when function is passed', () => {
        ...
        expect(findDataTest(component, 'save-bike').length).toBe(0);
        expect(findDataTest(component, 'delete-bike').length).toBe(1);
        expect(findDataTest(component, 'add-bike-part').length).toBe(1);
    });
